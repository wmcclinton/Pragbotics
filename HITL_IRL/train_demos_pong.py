...
def main(argv):
  ...
  num = 3
  demos = pickle.load(open( "demos_" + str(num) + ".p", "rb" ))

  policy = train_policy(demos, policy)

  for i in range(5):
    traj = gen_sample_traj(policy, max_steps=500, path="trained_policy_vids/video_" + str(num) + "_" + str(i) + ".mp4", record=True)

  ###

  save_policy(policy, 'agents/trained_policy_demos_' + str(num))
