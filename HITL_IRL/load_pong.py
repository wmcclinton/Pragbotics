...
def main(argv):
  ...
  load_agents = True
  get_demos = False
  train_on_demos = False

  if load_agents:
    policy = load_policy('agents/BC_trained_policy1_2_demos')
    traj = gen_sample_traj(policy, max_steps=500, path='vids/video_of_BC_policy.mp4', record=True)
    #print(traj)
    print("Loaded Agent")
  else:
